...
<div class="card">
  ...
  <div id="uploadScreen">
    <h1>かんたんWeb公開</h1>
    <p class="subtitle">Webサイトのフォルダ、またはZIPファイルを選ぶだけで、<br>すぐに公開できます。</p>
    ...
    <p class="hint">HTML / CSS / JavaScript / 画像などに対応（ZIP・HTML単体も可）</p>

    <div style="margin-top: 16px; display: flex; flex-direction: column; gap: 10px; align-items: center;">
      <button id="htmlBtn" type="button" class="action-btn btn-secondary">HTMLファイルを選択（1つだけ公開）</button>
      <button id="zipBtn" type="button" class="action-btn btn-secondary">ZIPファイルを選択</button>
    </div>
...
<input type="file" id="zipInput" accept=".zip,application/zip" class="hidden">
<input type="file" id="htmlInput" accept=".html,.htm,text/html" class="hidden">
...
(function () {
  ...
  zipInput.addEventListener('change', function (e) {
    var files = Array.prototype.slice.call(e.target.files);
    if (files.length === 0) return;
    startUpload([{ file: files[0], path: files[0].name }]);
    zipInput.value = '';
  });
  ...
  htmlInput.addEventListener('change', function (e) {
    var files = Array.prototype.slice.call(e.target.files);
    if (files.length === 0) return;
    startUpload([{ file: files[0], path: files[0].name, forceIndex: true }]);
    htmlInput.value = '';
  });
  ...
  function startUpload(entries) {
    ...
    var isZipUpload = entries.length === 1 && /\.zip$/i.test(entries[0].path);
    var isForcedIndex = entries.length === 1 && entries[0].forceIndex;

    if (!isZipUpload && !isForcedIndex) {
    ...
    entries.forEach(function (e) {
      var uploadPath = e.forceIndex ? 'index.html' : e.path;
      formData.append('files', e.file, uploadPath);
    });
...
async function handleUpload(request, env) {
  ...
  // { path, size, getBytes: () => Promise<Uint8Array|ArrayBuffer> } の配列に正規化する。
  // 通常の複数ファイル/フォルダアップロードと、ZIP1本のアップロードの両方をここで吸収する。
  let rawEntries;

  const isSingleZip =
    rawFiles.length === 1 &&
    rawFiles[0] instanceof File &&
    rawFiles[0].name.toLowerCase().endsWith(".zip");

  if (isSingleZip) {
    const zipFile = rawFiles[0];
    if (zipFile.size > MAX_TOTAL_SIZE) {
      return jsonError(
        `ZIPファイルが大きすぎます（最大 ${formatMB(MAX_TOTAL_SIZE)}）。`,
        400
      );
    }
    ...
    let zipEntries;
    try {
      zipEntries = await extractZipEntries(await zipFile.arrayBuffer());
    } catch (e) {
      return jsonError(
        "ZIPファイルを読み取れませんでした。ファイルが壊れているか、対応していない形式の可能性があります。",
        400
      );
    }

    if (zipEntries.length === 0) {
      return jsonError("ZIPファイルの中にファイルが見つかりませんでした。", 400);
    }

    rawEntries = zipEntries.map((z) => ({
      path: z.path,
      size: z.data.length,
      getBytes: async () => z.data,
    }));
  } else {
    rawEntries = [];
  ...
  for (const item of rawEntries) {
    ...
    const safePath = sanitizeRelativePath(item.path);
    if (safePath === null) {
    ...
    entries.push({ path: safePath, getBytes: item.getBytes });
  }
  ...
  try {
    for (const entry of strippedEntries) {
      ...
      await env.SITES_BUCKET.put(key, bytes, {
...
/* =========================================================
 * ZIP展開（外部ライブラリ不要。Workers標準のDecompressionStreamのみ使用）
 * ========================================================= */
async function extractZipEntries(arrayBuffer) {
  ...
  // End Of Central Directory (EOCD) を末尾側から探す
  const maxCommentLen = 65535;
  const searchStart = Math.max(0, len - 22 - maxCommentLen);
  let eocdOffset = -1;
  ...
  for (let i = 0; i < totalEntries; i++) {
    ...
    centralEntries.push({ filename, compressionMethod, compressedSize, localHeaderOffset });
    offset += 46 + filenameLen + extraLen + commentLen;
  }

  const results = [];
  for (const entry of centralEntries) {
    // ディレクトリ自体のエントリや、Mac圧縮時に混ざる余計なファイルはスキップ
    if (entry.filename.endsWith("/")) continue;
    if (entry.filename.startsWith("__MACOSX/")) continue;
    if (entry.filename.split("/").pop() === ".DS_Store") continue;

    const lh = entry.localHeaderOffset;
    if (view.getUint32(lh, true) !== 0x04034b50) {
      throw new Error("ZIPのローカルヘッダーが不正です");
    }
    ...
    if (entry.compressionMethod === 0) {
      ...
      data = await inflateRawDeflate(compressedData);
    } else {
      throw new Error("対応していない圧縮方式です");
    }

    results.push({ path: entry.filename, data });
  }
